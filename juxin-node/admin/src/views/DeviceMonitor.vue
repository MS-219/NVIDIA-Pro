  ...
  <div class="monitor-page">
    ...
    <div class="table-container">
      ...
      <el-table :data="devices" border stripe v-loading="loading" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="节点运行环境" min-width="220">
          <template #default="{ row }">
            <div style="font-size: 12px; color: #666;">
              <div>{{ row.deviceModel || 'RK3588 型号待上报' }}</div>
              <div class="runtime-version">{{ row.architecture || 'aarch64' }} · Linux {{ row.l4tVersion || '版本待上报' }}</div>
...
const exportDeviceCsv = async () => {
  ...
  try {
    ...
    anchor.href = url
    anchor.download = `聚芯节点_设备清单_${new Date().toISOString().slice(0, 10)}.csv`
...
const deviceBindUrl = (record) => {
  ...
  return `https://jd.ldjuxin.yun/bind?code=${encodeURIComponent(code)}`
}
...
const drawQrCard = async (record) => {
  ...
  context.fillText('节点', 72 + brandWidth + 18, 118)

  context.fillStyle = '#a9afb6'
  context.font = '500 42px Arial, sans-serif'
  context.fillStyle = '#5f9800'
  context.font = '700 30px Arial, sans-serif'
  context.fillText('RK3588', 760, 110)
  ...
  context.fillText('打开聚芯节点 APP，进入设备页面扫码', 540, 1186)
  ...
  context.fillText('聚芯节点 · RK3588 边缘设备', 540, 1398)
...
const downloadQrArchive = async () => {
  ...
  try {
    ...
    const anchor = document.createElement('a')
    anchor.href = url
    anchor.download = `聚芯节点_设备二维码_${new Date().toISOString().slice(0, 10)}.zip`
    document.body.appendChild(anchor)
    anchor.click()
    anchor.remove()
    URL.revokeObjectURL(url)
