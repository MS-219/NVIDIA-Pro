...
public class EdgeDeviceController {
    ...
    public Result<Map<String, Object>> reportStatus(@RequestBody Map<String, Object> report,
            ...
            @RequestHeader(value = IEdgeDeviceAccessService.RK3588_DEVICE_TOKEN_HEADER, required = false) String rk3588DeviceToken) {
        Device authenticatedDevice = edgeDeviceAccessService.authenticate(firstToken(deviceToken, rk3588DeviceToken));
        if (report == null) {
        ...
        String cpuUsage = firstText(report, "cpu_load", "actual_cpu_load", "0");
        String memUsage = firstText(report, "mem_load", "actual_mem_load", "0");
    ...
    // Source-compatible overloads retained for original backend integrations.
    public Result<Map<String, Object>> reportStatus(
            Map<String, String> report, HttpServletRequest request, String deviceToken) {
        Map<String, Object> converted = new HashMap<>();
        if (report != null) converted.putAll(report);
        return reportStatus(converted, request, deviceToken, null);
    }

    public Result<ComputeJob> fetchTask(String sn, String deviceToken) {
        return fetchTask(sn, deviceToken, null);
    }

    public Result<String> submitResult(ComputeJob result, String deviceToken) {
        return submitResult(result, deviceToken, null);
    }

    public Result<String> submitCommandResult(Map<String, Object> result, String deviceToken) {
        return submitCommandResult(result, deviceToken, null);
    }

    private String firstToken(String primary, String rk3588) {
        return primary != null && !primary.isBlank() ? primary : rk3588;
    }

    private String firstText(Map<String, Object> report, String primary, String fallback, String defaultValue) {
        String value = getReportText(report, primary);
        if (value == null) value = getReportText(report, fallback);
